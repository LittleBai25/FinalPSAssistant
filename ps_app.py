                ...
                if st.button("生成院校信息报告", disabled=not generate_enabled, key="generate_uni_info", use_container_width=True):
                    if university and major:
                        ...
                        with collection_container:
                            ...
                            # Display Agent 1.1 report with confirmation button
                            with collection_container:
                                st.success("初步信息收集已完成")
                                st.markdown("### 初步信息收集报告")
                                st.markdown(report)
                                
                                # Only show confirmation section if there are missing fields to complete
                                if missing_fields and urls_for_deep:
                                    ...
                                    with col2:
                                        if st.button("跳过补充收集", key="skip_agent2", use_container_width=True):
                                            # Store the final report without running Agent 1.2
                                    ...
                                    # No missing fields, so no need for Agent 1.2
